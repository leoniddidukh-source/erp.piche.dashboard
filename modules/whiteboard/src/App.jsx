...
function App() {
  ...
  const applyShapeUpdate = useCallback(
    (updatedShape, description, options) => {
      ...
      setShapes((prev) =>
        prev.map((shape) =>
          shape.id === normalizedShape.id ? normalizedShape : shape,
        ),
      )
      broadcast({
        kind: 'update-shape',
  ...
  const updateTableCell = useCallback(
    (tableShape, row, col, value, options) => {
      ...
      applyShapeUpdate(updatedShape, `${userName} updated cell R${row + 1}C${col + 1}`)
    },
  ...
  const handleTextEditorChange = useCallback((event) => {
      ...
      setCellEditor((current) => {
        ...
        if (shouldCommit) {
          ...
          const tableShape = shapesRef.current.find(
            (shape) =>
              shape.id === current.tableId && shape.type === 'table',
          )
          if (tableShape) {
            updateTableCell(tableShape, current.row, current.col, trimmed, {
              color,
            })
          }
        }
  ...
  const handlePointerMove = (event) => {
        ...
        prev.map((shape) => {
          ...
          if (original.type === 'line') {
            ...
            return {
              ...
              x1: clamp(
                minX +
                  (original.x1 - originalBounds.minX) * scaleXLine,
                0,
                position.bounds.width,
              ),
              y1: clamp(
                minY +
                  (original.y1 - originalBounds.minY) * scaleYLine,
                0,
                position.bounds.height,
              ),
              x2: clamp(
                minX +
                  (original.x2 - originalBounds.minX) * scaleXLine,
                0,
                position.bounds.width,
              ),
              y2: clamp(
                minY +
                  (original.y2 - originalBounds.minY) * scaleYLine,
                0,
                position.bounds.height,
              ),
            }
    ...
    if (draggingShapeRef.current) {
      const position = getPointerPosition(event.clientX, event.clientY)
      if (!position) return
      const { id, size, offset } = draggingShapeRef.current
        ...
        prev.map((shape) => {
          ...
          if (shape.type === 'path') {
            const targetMinX = clamp(
              nextX,
              0,
              Math.max(bounds.width - size.width, 0),
            )
          ...
          if (shape.type === 'circle') {
            return {
              ...shape,
              cx: clamp(
                point.x - offset.x + size.width / 2,
                shape.r,
                bounds.width - shape.r,
              ),
              cy: clamp(
                point.y - offset.y + size.height / 2,
                shape.r,
                bounds.height - shape.r,
              ),
            }
          }
          if (shape.type === 'ellipse') {
            return {
              ...
              cx: clamp(
                point.x - offset.x + size.width / 2,
                shape.rx,
                bounds.width - shape.rx,
              ),
              cy: clamp(
                point.y - offset.y + size.height / 2,
                shape.ry,
                bounds.height - shape.ry,
              ),
            }
          }
          if (shape.type === 'line') {
      ...
      case 'table': {
        ...
        for (let row = 0; row < shape.rows; row += 1) {
          for (let col = 0; col < shape.cols; col += 1) {
            ...
            const cy =
              shape.y + row * shape.cellHeight + shape.cellHeight / 2 + 4
            textNodes.push(
              <text
                key={`cell-${row}-${col}`}
                x={cx}
